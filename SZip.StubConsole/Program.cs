...
namespace SZip.StubConsole;

/// <summary>
/// Cross-platform (Windows/Linux/macOS) headless self-extractor. Reads the SZip footer from
/// the end of its own executable and extracts the payload. Usage:
///   SZip.Stub [target-dir] [-p password] [-y]
...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        SZipFooter? footer = SfxComposer.ReadFooter(selfPath);
        if (footer == null)
        {
            Console.Error.WriteLine("Tệp này không chứa dữ liệu nén SZip.");
            return 1;
        }
        ...
        Console.WriteLine($"SZip - Trình giải nén [{footer.Method}]"
        ...
        try
        {
            ...
            {
                SZipEngine.Unpack(payload, target, footer.Method,
    ...
    {
        Console.WriteLine("""
SZip self-extractor (đa nền tảng)
