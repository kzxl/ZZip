...
namespace ZZip.Stub.Services
{
    /// <summary>
    /// Runtime extractor for an SFX executable. Reads the ZZip footer from the end of the
    ...
    public class ExtractionService
    {
        ...
        /// <summary>
        /// Detects whether this executable carries an ZZip payload and produces a
        ...
        public bool CheckPayload(out string statusMessage)
        {
            ...
            if (_footer == null)
            {
                statusMessage = "Trống rỗng. Tệp này không chứa dữ liệu nén ZZip.";
                return false;
            }
        ...
        {
            _footer ??= SfxComposer.ReadFooter(_selfPath)
                ?? throw new InvalidOperationException("Không có dữ liệu ZZip để giải nén.");
