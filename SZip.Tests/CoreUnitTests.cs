...
namespace SZip.Tests
{
    public class FooterTests
    {
        ...
        public void RoundTrips_AllFields()
        {
            ...
            {
                Flags = SZipFlags.Appended | SZipFlags.Encrypted | SZipFlags.Precompressed,
            ...
            var read = SZipFooter.ReadFromEnd(ms);
            Assert.NotNull(read);
        ...
        {
            var footer = new SZipFooter { Flags = SZipFlags.Appended };
            using var ms = new MemoryStream();
            footer.Write(ms);
            byte[] all = ms.ToArray();

            Assert.Equal(SZipFooter.Size, all.Length);
            byte[] tail = all[^8..];
            // 'S','Z','I','P','S','F','X',0x01
            Assert.Equal(new byte[] { 0x53, 0x5A, 0x49, 0x50, 0x53, 0x46, 0x58, 0x01 }, tail);
        }
        ...
        public void ReturnsNull_WhenNoMagic()
        {
            ...
            Assert.Null(SZipFooter.ReadFromEnd(ms));
        }
