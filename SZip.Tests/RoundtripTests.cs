    ...
    public class RoundtripTests
    {
        ...
        public void Appended_AllCodecs_RoundTrip(CompressionMethod method)
        {
            ...
            using (var payload = SfxComposer.OpenPayload(exe, footer))
                Assert.True(SZipEngine.VerifyCrc(payload, footer.Crc32));

            string outDir = ws.At("ex");
            using (var payload = SfxComposer.OpenPayload(exe, footer))
                SZipEngine.Unpack(payload, outDir, footer.Method, null, footer.IsPrecompressed);
        ...
        public void Encrypted_CorrectPassword_RoundTrips()
        {
            ...
            string outDir = ws.At("ex");
            using (var payload = SfxComposer.OpenPayload(exe, footer))
                SZipEngine.Unpack(payload, outDir, footer.Method, "S3cr3t!", false);
        ...
        public void Encrypted_WrongPassword_Throws()
        {
            ...
            {
                using var payload = SfxComposer.OpenPayload(exe, footer);
                SZipEngine.Unpack(payload, ws.At("ex"), footer.Method, "wrong-password", false);
            });
        }
        ...
        public void CorruptedPayload_FailsCrc()
        {
            ...
            using var payload = SfxComposer.OpenPayload(exe, footer);
            Assert.False(SZipEngine.VerifyCrc(payload, footer.Crc32));
        }
    }
}
