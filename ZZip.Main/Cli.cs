...
internal static class Cli
{
    ...
    private static int RunCompress(string[] args)
    {
        ...
        {
            Console.WriteLine($"ZZip: Nén chuẩn ZIP cho \"{source}\" -> \"{output}\"");
        ...
        if (method == null)
        {
            ...
            Console.WriteLine($"[ZZip Auto-Detect] Nhận diện: [{classified.DetectedType}] -> Áp dụng {classified.RecommendedMethod} (Level {options.Level})");
        ...
        if (usePrecomp)
        {
            ...
            if (exe == null)
                return Fail("Bật --precomp nhưng không tìm thấy precomp.exe (đặt cạnh ZZip.exe, trong tools\\, hoặc PATH).");
            Console.WriteLine($"  precomp: {exe}");
        }

        bool isSfx = output.EndsWith(".exe", StringComparison.OrdinalIgnoreCase);

        Console.WriteLine($"ZZip: nén \"{source}\"");
    ...
    {
        if (args.Length < 2) return Fail("Cú pháp: ZZip zip <nguồn> [-o <đích.zip>]");
        ...
        Console.WriteLine($"ZZip: Nén chuẩn PKZIP \"{source}\" -> \"{output}\"");
        StandardZip.CompressToZip(source, output);
        Console.WriteLine($"✔ Hoàn tất nén ZIP: \"{output}\"");
        return 0;
    }
    ...
    private static int RunEstimate(string[] args)
    {
        ...
        string source = args[1];
        Console.WriteLine($"ZZip: ước tính tỉ lệ nén cho \"{source}\"...");
    ...
    private static int RunExtract(string[] args)
    {
        ...
        if (footer == null) return Fail("Tệp không phải SFX ZZip hợp lệ.");

        if (footer.IsEncrypted && string.IsNullOrEmpty(password))
            return Fail("Gói được mã hóa. Dùng -p <mật_khẩu>.");

        Console.WriteLine($"ZZip: giải nén \"{source}\" -> \"{dest}\"  [{footer.Method}]"
    ...
    private static int RunInfo(string[] args)
    {
        ...
        if (f == null) return Fail("Tệp không phải SFX ZZip hợp lệ.");
    ...
    private static int RunTest(string[] args)
    {
        ...
        Console.WriteLine($"ZZip: kiểm tra toàn vẹn \"{source}\" [{footer.Method}]"
            + (footer.IsEncrypted ? " (mã hóa)" : ""));
    ...
    {
        Console.WriteLine("""
ZZip - Trình siêu nén SFX

Cách dùng:
  ZZip c <nguồn> [-o ra.exe] [-m zstd|lzma|brotli|store]
                 [--ultra|--normal|--fast] [--level N] [--window N]
                 [--threads N] [--split KÍCH_THƯỚC] [-p mật_khẩu] [--zip]
  ZZip x <sfx.exe> [-o thư_mục] [-p mật_khẩu]
  ZZip i <sfx.exe>
  ZZip l <sfx.exe> [-p mật_khẩu]   (liệt kê nội dung, không giải nén)
  ZZip t <sfx.exe> [-p mật_khẩu]   (kiểm tra toàn vẹn, không ghi đĩa)
  ZZip e <nguồn>                 (ước tính nhanh tỉ lệ nén)
  ZZip h

Ví dụ:
  ZZip c "C:\Data" -o Data.exe --ultra
  ZZip c game.iso -m lzma --level 22 --split 2GB
  ZZip c secret\ -o s.exe -p "MatKhau123" --zip
  ZZip l Data.exe
  ZZip t Data.exe
  ZZip e "C:\Data"
  ZZip x Data.exe -o C:\Out
...
""");
    ...
    private static int RunRegisterContextMenu(string[]? args = null)
    {
        ...
        Console.WriteLine($"Đang đăng ký ZZip vào Windows Explorer context menu (Ngôn ngữ: {LocalizationService.CurrentLanguageCode})...");
        if (ShellContextMenuService.Register())
        {
            Console.WriteLine("Thành công: Đã thêm các mục ZZip vào menu chuột phải theo ngôn ngữ hiện hành.");
            return 0;
        }
    ...
    {
        Console.WriteLine("Đang gỡ bỏ ZZip khỏi Windows Explorer context menu...");
        if (ShellContextMenuService.Unregister())
        {
            Console.WriteLine("Thành công: Đã gỡ bỏ toàn bộ liên kết ZZip khỏi context menu.");
            return 0;
        }
