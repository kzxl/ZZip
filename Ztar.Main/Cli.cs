...
internal static class Cli
{
    ...
    private static int RunCompress(string[] args)
    {
        ...
        if (usePrecomp)
        {
            ...
            if (exe == null)
                return Fail("Bật --precomp nhưng không tìm thấy precomp.exe (đặt cạnh Ztar.exe, trong tools\\, hoặc PATH).");
            Console.WriteLine($"  precomp: {exe}");
        }

        Console.WriteLine($"Ztar: nén \"{source}\"");
    ...
    private static int RunEstimate(string[] args)
    {
        ...
        Console.WriteLine($"Ztar: ước tính tỉ lệ nén cho \"{source}\"...");
    ...
    private static int RunExtract(string[] args)
    {
        ...
        if (footer == null) return Fail("Tệp không phải SFX Ztar hợp lệ.");

        if (footer.IsEncrypted && string.IsNullOrEmpty(password))
            return Fail("Gói được mã hóa. Dùng -p <mật_khẩu>.");

        Console.WriteLine($"Ztar: giải nén \"{source}\" -> \"{dest}\"  [{footer.Method}]"
    ...
    private static int RunInfo(string[] args)
    {
        ...
        if (f == null) return Fail("Tệp không phải SFX Ztar hợp lệ.");
    ...
    private static int RunTest(string[] args)
    {
        ...
        Console.WriteLine($"Ztar: kiểm tra toàn vẹn \"{source}\" [{footer.Method}]"
            + (footer.IsEncrypted ? " (mã hóa)" : ""));
...
Cách dùng:
  ...
  Ztar i <sfx.exe>
  Ztar l <sfx.exe> [-p mật_khẩu]   (liệt kê nội dung, không giải nén)
  Ztar t <sfx.exe> [-p mật_khẩu]   (kiểm tra toàn vẹn, không ghi đĩa)
  Ztar e <nguồn>                 (ước tính nhanh tỉ lệ nén)
  Ztar h

Ví dụ:
  Ztar c "C:\Data" -o Data.exe --ultra
  Ztar c game.iso -m lzma --level 22 --split 2GB
