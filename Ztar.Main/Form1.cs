...
namespace Ztar.Main
{
    public class Form1 : Form
    {
        ...
        private Ztar.Core.CompressionMethod SelectedMethod() => cmbMethod.SelectedIndex switch
        {
            1 => Ztar.Core.CompressionMethod.Lzma,
            2 => Ztar.Core.CompressionMethod.Brotli,
            3 => Ztar.Core.CompressionMethod.Store,
            _ => Ztar.Core.CompressionMethod.Zstd,
        };

        private Ztar.Core.CompressionProfile SelectedProfile() => cmbProfile.SelectedIndex switch
        {
            0 => Ztar.Core.CompressionProfile.Fast,
            1 => Ztar.Core.CompressionProfile.Normal,
            _ => Ztar.Core.CompressionProfile.Ultra,
        };
        ...
        private async void BtnCompress_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var options = Ztar.Core.CompressionOptions.FromProfile(profile, method);
                options.Password = password;
                options.UsePrecomp = usePrecomp;
                if (longMode)
                {
                    options.LongDistanceMatching = true;
                    if (options.WindowLog < Ztar.Core.CompressionOptions.MaxLongWindowLog)
                        options.WindowLog = Ztar.Core.CompressionOptions.MaxLongWindowLog;
                }
