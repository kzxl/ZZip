...
namespace SZip.Stub.Services
{
    /// <summary>
    /// Runtime extractor for an SFX executable. Reads the SZip footer from the end of the
    /// running .exe, then streams the appended (or multi-part) payload through the shared
    /// <see cref="SZipEngine"/>. No embedded resources, no temp files.
    /// </summary>
    public class ExtractionService
    {
        private readonly string _selfPath;
        private SZipFooter? _footer;
        ...
        public SZipFooter? Footer => _footer;

        /// <summary>
        /// Detects whether this executable carries an SZip payload and produces a
        ...
        public bool CheckPayload(out string statusMessage)
        {
            ...
            if (_footer == null)
            {
                statusMessage = "Trống rỗng. Tệp này không chứa dữ liệu nén SZip.";
                return false;
            }
        ...
        /// </summary>
        /// <param name="password">Required when <see cref="SZipFooter.IsEncrypted"/> is set.</param>
        public void ExtractPayload(string destinationPath, string? password = null, IProgress<long>? progress = null)
        {
            _footer ??= SfxComposer.ReadFooter(_selfPath)
                ?? throw new InvalidOperationException("Không có dữ liệu SZip để giải nén.");
            ...
            {
                if (!SZipEngine.VerifyCrc(verifyStream, _footer.Crc32))
